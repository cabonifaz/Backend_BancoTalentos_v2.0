...
public class PromptBuilder {
  ...
  public String buildCVPrompt(String extractedText) {
    return """
        ...
        El campo `funciones` NUNCA debe superar los 200 caracteres. Si el texto original supera los 200 caracteres,
        DEBES resumirlo SÍ O SÍ a un máximo de 200 caracteres, conservando la esencia y sin omitir las tecnologías
        o herramientas técnicas mencionadas. Esta regla se aplica a TODAS las experiencias, no solo a partir de la 10ma.
        No devuelvas jamás un `funciones` de más de 200 caracteres.

        Para las funciones laborales, cuando NO se supere el límite de 200 caracteres, incluye TANTO el párrafo
        descriptivo inicial COMO todos los puntos o bullets que aparezcan bajo la experiencia, concatenados en un
        solo string separados por salto de línea (\n). Si al incluir todo se superan los 200 caracteres, entonces
        aplica el LÍMITE OBLIGATORIO y resume a un máximo de 200 caracteres (el límite manda sobre incluir todos los bullets).
        ...
        - El campo `presentacion` debe ser como aparece en el CV, sin traducir, MIENTRAS no supere los 200 caracteres.
          LÍMITE OBLIGATORIO: `presentacion` NUNCA debe superar los 200 caracteres. Si el texto del CV supera los 200
          caracteres, DEBES resumirlo SÍ O SÍ a un máximo de 200 caracteres conservando su esencia. No devuelvas jamás
          una `presentacion` de más de 200 caracteres.
